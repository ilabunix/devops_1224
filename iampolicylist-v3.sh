...
while IFS= read -r ROLE_NAME; do
  ...
  echo "$ATTACHED_POLICIES" | grep -o '"PolicyName":"[^"]*","PolicyArn":"[^"]*"' | while IFS= read -r POLICY; do
    ...
    # Save attached policy to a file in the current directory
    if [ -n "$POLICY_DOCUMENT" ]; then
      echo "$POLICY_DOCUMENT" > "${ROLE_NAME}_attached_${POLICY_NAME}.json"
      echo "  Saved attached policy: ${ROLE_NAME}_attached_${POLICY_NAME}.json"
    else
  ...
  echo "$INLINE_POLICY_NAMES" | sed -e 's/[][]//g' -e 's/"//g' | tr ',' '\n' | while IFS= read -r INLINE_POLICY; do
    if [ -n "$INLINE_POLICY" ]; then
      ...
      # Save inline policy to a file in the current directory
      if [ -n "$INLINE_POLICY_DOCUMENT" ]; then
        echo "$INLINE_POLICY_DOCUMENT" > "${ROLE_NAME}_inline_${INLINE_POLICY}.json"
        echo "  Saved inline policy: ${ROLE_NAME}_inline_${INLINE_POLICY}.json"
      else
...
echo "All policies have been saved in the current directory."