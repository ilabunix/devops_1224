...
while IFS= read -r ROLE_NAME; do
  ...
  ATTACHED_POLICIES=$(aws iam list-attached-role-policies --role-name "$ROLE_NAME" --query 'AttachedPolicies[*]' --output json)
  ...
  echo "$INLINE_POLICY_NAMES" | sed -e 's/[][]//g' -e 's/"//g' | tr ',' '\n' | while IFS= read -r INLINE_POLICY; do
    if [ -n "$INLINE_POLICY" ]; then
      INLINE_POLICY_DOCUMENT=$(aws iam get-role-policy --role-name "$ROLE_NAME" --policy-name "$INLINE_POLICY" --query 'PolicyDocument' --output json)
